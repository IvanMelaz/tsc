...
/*!50001 VIEW `vw_CentraleUtente` AS select `tsc_user`.`name` AS `tsc_username`,`tsc_group`.`name` AS `centrale` from ((`tsc_group` join `tsc_group_user_link` on((`tsc_group`.`groupid` = `tsc_group_user_link`.`groupid`))) join `tsc_user` on((`tsc_group_user_link`.`uid` = `tsc_user`.`uid`))) */;
