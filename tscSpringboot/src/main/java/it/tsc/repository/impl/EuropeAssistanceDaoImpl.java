...
public class EuropeAssistanceDaoImpl extends BaseDao implements EuropeAssistanceDao {
  private static Logger logger = LoggerFactory.getLogger(EuropeAssistanceDaoImpl.class);
   ...
   * (non-Javadoc)
   *
   * @see it.tsc.dao.EuropeAssistanceDao#saveAllarm(it.tsc.domain. AllarmiEuropeAssistance)
   */
  @Override
  public void saveAllarm(AllarmiEuropeAssistance allarmiEuropeAssistance) {
    ...
    try {
      ...
      logger.debug("saveAllarm: {}", allarmiEuropeAssistance.getId_allarme());
      tx.commit();
    } catch (Exception e) {
      tx.rollback();
  ...
  public String listJsonAllarmiOpen() {
    ...
    TypedQuery<AllarmiEuropeAssistance> query = entityManager
        .createNamedQuery(AllarmiEuropeAssistance.SELECT_ALLARM_EA, AllarmiEuropeAssistance.class);
