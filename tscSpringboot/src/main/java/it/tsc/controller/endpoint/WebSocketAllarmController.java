...
public class WebSocketAllarmController {
	...
	@OnOpen
	public void onOpenSession(@PathParam("userid") String userid,
			Session session, EndpointConfig endpointConfig) {
		/**
		 * Manage session
		 */
		HttpSession httpSession = (HttpSession) endpointConfig
				.getUserProperties().get("HTTP_SESSION");
		...
		if (httpSession != null) {
			...
			logger.debug("manage session attribute {} session id: {} user: {}",
					session, session.getId(), userid);
		}
		synchronized (clients) {
			clients.add(session);
			logger.debug("add item to clients: {} clients size: {}",
					session.getId(), clients.size());
			this.endpointConfig.getUserProperties().put(session.getId(),
					userid); // store mapping of
			// WebSocket
		}
		...
		if (clients.size() != 0) {
			...
			scheduledExecutorService.scheduleAtFixedRate(
					() -> checkAllarmOnDatabase(session), 0, 3,
					TimeUnit.SECONDS);
		}
	}
