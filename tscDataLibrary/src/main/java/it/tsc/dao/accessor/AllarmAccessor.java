...
public interface AllarmAccessor {
	@Query("INSERT INTO ks_tsc.tb_allarmi (ab_codi,data_arrivo,evento,id_allarme,user) VALUES (:ab_codi,:data_arrivo,:evento,:id_allarme,:user) IF NOT EXISTS;")
	...
	@Query("DELETE FROM ks_tsc.tb_allarmi WHERE id_allarme = :id_allarme;")
	public void removeAllarme(@Param("id_allarme") String id_allarme);

	@Query("UPDATE ks_tsc.tb_allarmi SET user=:user WHERE id_allarme = :id_allarme IF EXISTS;")
